...
cartDetails.addEventListener('click', (e) => {
    ...
    const ManagerStore = (() => {
        ...
        function viewStoreDashboard(storeId) {
            ...
            // Show Payment Manager (Store View) - Modern Dashboard Card Style
            const modal = document.getElementById('store-payment-manager-modal');
            if (modal) {
                document.getElementById('store-payment-manager-content').innerHTML = '';
                renderStorePaymentManager(storeId);
                modal.style.display = 'flex';
            } else {
        ...
        // ===== Payment Manager (Store View) & Payment Stores (Admin View) =====
        function renderStorePaymentManager(storeId) {
            const store = activeStores.find(s => s.id === storeId);
            if (!store) return;

            const container = document.getElementById('store-payment-manager-content');
            if (!container) return;
                    ...
                    <div class="payment-cards-grid">
                        <div class="payment-card status-card">
                            ...
                            <div class="card-content">
                                <h3>สถานะร้านค้า</h3>
                                <p class="status-value ${store.status === 'active' ? 'active' : 'locked'}">${store.status === 'active' ? '✓ เปิดใช้งาน' : '🔒 ถูกล็อค'}</p>
                            </div>
                        </div>

                        <div class="payment-card time-card">
                            <div class="card-icon">⏰</div>
                            <div class="card-content">
                                <h3>เวลาคงเหลือ</h3>
                                <p class="time-value">${days} วัน ${hours} ชม. ${minutes} นาที</p>
                                <div class="time-progress">
                                    <div class="progress-bar" style="width: ${Math.max(0, (timeRemaining / (30 * 24 * 60 * 60 * 1000)) * 100)}%"></div>
                                </div>
                            </div>
                        </div>

                        <div class="payment-card amount-card">
                            <div class="card-icon">💰</div>
                            <div class="card-content">
                                <h3>ยอดค่าระบบต่อรอบ</h3>
                                <p class="amount-value">฿${(store.currentPaymentAmount || 0).toLocaleString()}</p>
                                <p class="period-text">รอบบิล: ${store.billingCycle || 'รายเดือน'}</p>
                            </div>
                        </div>

                        <div class="payment-card expiry-card">
                            <div class="card-icon">📅</div>
                            <div class="card-content">
                                <h3>วันหมดอายุ</h3>
                                <p class="expiry-value">${new Date(store.expiryDate).toLocaleDateString('th-TH')}</p>
                            </div>
                        </div>
                    </div>

                    <div class="payment-actions-section">
                        <h3>📤 แนบหลักฐานการชำระเงิน</h3>
                        <div class="payment-upload-form">
                            <div class="form-group">
                                <label>ยอดเงินที่ชำระ</label>
                                <input type="number" id="payment-amount-input" placeholder="กรอกจำนวนเงิน" value="${store.currentPaymentAmount || ''}">
                            </div>
                            <div class="form-group">
                                <label>ลิงก์หรือรูปภาพหลักฐาน</label>
                                <input type="url" id="payment-proof-input" placeholder="https://... หรือ URL รูปภาพ">
                            </div>
                            <button type="button" class="btn btn-primary" onclick="ManagerStore.attachPaymentProof(${store.id})">
                    ...
                    <div class="payment-history-section">
                        ...
                        ${latestPayment ? `
                            <div class="latest-payment-info">
                                <p><strong>การชำระครั้งล่าสุด:</strong> ${new Date(latestPayment.submittedAt).toLocaleString('th-TH')}</p>
                                <p><strong>จำนวนเงิน:</strong> ฿${latestPayment.amount.toLocaleString()}</p>
                                <p><strong>สถานะ:</strong> <span class="status-badge ${latestPayment.status}">${latestPayment.status === 'pending' ? 'รอตรวจสอบ' : (latestPayment.status === 'approved' ? 'อนุมัติแล้ว' : 'ปฏิเสธ')}</span></p>
                            </div>
                        ` : '<p style="text-align: center; opacity: 0.7;">ยังไม่มีประวัติการชำระเงิน</p>'}
                    </div>
                </div>
            `;
        }

        function renderPaymentStoresAdmin() {
            // สร้างรายการร้านค้าพร้อมข้อมูล payment
            const container = document.getElementById('payment-stores-admin-content');
            if (!container) return;

            const activeStoresWithPayment = activeStores.map(store => {
                const latestPayment = paymentHistory
                ...
                return {
                    ...store,
                    latestPayment,
                    currentPaymentAmount: store.currentPaymentAmount || 0
                };
            });

            container.innerHTML = `
                <div class="payment-admin-section">
                    <div class="admin-payment-summary">
                        <h3>💰 สรุปรายได้</h3>
                        <div class="summary-cards">
                            <div class="summary-card">
                                <h4>รายได้รวม</h4>
                                <p class="amount">฿${paymentHistory.filter(p => p.status === 'approved').reduce((sum, p) => sum + p.amount, 0).toLocaleString()}</p>
                            </div>
                            <div class="summary-card">
                                <h4>รอตรวจสอบ</h4>
                                <p class="amount">฿${paymentHistory.filter(p => p.status === 'pending').reduce((sum, p) => sum + p.amount, 0).toLocaleString()}</p>
                            </div>
                            <div class="summary-card">
                                <h4>จำนวนร้านค้า</h4>
                                <p class="amount">${activeStores.length} ร้าน</p>
                            </div>
                        </div>
                    </div>

                    <div class="payment-stores-list">
                        <h3>🏪 ร้านค้าทั้งหมด</h3>
                        <div class="stores-payment-grid">
                            ${activeStoresWithPayment.map(store => `
                                <div class="store-payment-card">
                                    <div class="store-header">
                                        <h4>${store.shopName}</h4>
                                        <span class="package-badge ${store.packageType}">${store.packageType === 'premium' ? '⭐ Premium' : '📦 Standard'}</span>
                                    </div>
                                    
                                    <div class="store-payment-info">
                                        <div class="info-row">
                                            <span>ยอดค่าระบบต่อรอบ:</span>
                                            <span class="amount">฿${store.currentPaymentAmount.toLocaleString()}</span>
                                        </div>
                                        <div class="info-row">
                                            <span>รอบบิล:</span>
                                            <span>${store.billingCycle || 'รายเดือน'}</span>
                                        </div>
                                        <div class="info-row">
                                            <span>การชำระล่าสุด:</span>
                                            <span>${store.latestPayment ? new Date(store.latestPayment.submittedAt).toLocaleDateString('th-TH') : 'ยังไม่มี'}</span>
                                        </div>
                                        <div class="info-row">
                                            <span>สถานะการชำระ:</span>
                                            <span class="status-badge ${store.latestPayment?.status || 'none'}">
                                                ${store.latestPayment ? (store.latestPayment.status === 'pending' ? 'รอตรวจสอบ' : (store.latestPayment.status === 'approved' ? 'อนุมัติแล้ว' : 'ปฏิเสธ')) : 'ยังไม่มี'}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="store-payment-actions">
                                        <div class="form-group">
                                            <label>แจ้งยอดชำระใหม่</label>
                                            <div class="input-group">
                                                <input type="number" id="new-amount-${store.id}" placeholder="จำนวนเงิน" value="${store.currentPaymentAmount}">
                                                <button class="btn btn-small btn-primary" onclick="ManagerStore.updateStorePaymentAmount(${store.id})">แจ้งยอด</button>
                                            </div>
                                        </div>
                                        ${store.latestPayment && store.latestPayment.status === 'pending' ? `
                                            <div class="action-buttons">
                                                <button class="btn btn-success btn-small" onclick="ManagerStore.approvePayment(${store.latestPayment.id})">✓ อนุมัติ</button>
                                                <button class="btn btn-danger btn-small" onclick="ManagerStore.rejectPayment(${store.latestPayment.id})">✗ ปฏิเสธ</button>
                                                <button class="btn btn-info btn-small" onclick="ManagerStore.showPaymentProof(${store.latestPayment.id})">👁️ ดูหลักฐาน</button>
                                            </div>
                                        ` : ''}
                                    </div>
                                </div>
        ...
        function syncPaymentData() {
            // Two-way sync ข้อมูล payment ระหว่าง Admin และ Store
            activeStores.forEach(store => {
                ...
                if (latestPayment) {
                    store.currentPaymentAmount = latestPayment.amount;
                    store.lastPaymentDate = latestPayment.submittedAt;
                    store.paymentStatus = latestPayment.status;
                }
            });
        ...
        function updateStorePaymentAmount(storeId) {
            const amountInput = document.getElementById(`new-amount-${storeId}`);
            if (!amountInput) return;

            const amount = parseFloat(amountInput.value);
            if (!amount || amount <= 0) {
                Notify.error('จำนวนเงินไม่ถูกต้อง', 'กรุณากรอกจำนวนเงินที่ถูกต้อง');
                return;
            }
            ...
            store.currentPaymentAmount = amount;
            store.billingCycle = 'รายเดือน'; // Default cycle
            ...
            fetch(API_UPDATE_STORE_ENDPOINT, {
              ...
              .then(data => console.log('Payment amount updated:', data))
              .catch(err => console.error('Update payment amount error:', err));

            saveManagerData();
            renderPaymentStoresAdmin();

            Notify.success('แจ้งยอดสำเร็จ', `แจ้งยอดค่าระบบ ${amount.toLocaleString()} บาท ให้ร้าน "${store.shopName}" เรียบร้อยแล้ว`);
        }

        function attachPaymentProof(storeId) {
            const amountInput = document.getElementById('payment-amount-input');
            const proofInput = document.getElementById('payment-proof-input');

            if (!amountInput || !proofInput) return;

            const amount = parseFloat(amountInput.value);
            const proofUrl = proofInput.value.trim();
            ...
            if (!proofUrl) {
                Notify.error('หลักฐานการชำระเงิน', 'กรุณาแนบลิงก์หรือรูปภาพหลักฐาน');
                return;
            }
            ...
            // อัปเดตข้อมูลร้านค้า
            store.currentPaymentAmount = amount;
            store.lastPaymentSubmission = payment.submittedAt;
            ...
            fetch(API_UPDATE_STORE_ENDPOINT, {
              ...
              .then(data => console.log('Payment proof submitted:', data))
              .catch(err => console.error('Submit payment proof error:', err));

            renderStorePaymentManager(storeId);
            renderPaymentStoresAdmin();
            saveManagerData();

            Notify.success('ส่งหลักฐานสำเร็จ', 'หลักฐานการชำระเงินถูกส่งเรียบร้อยแล้ว รอการตรวจสอบ');
        ...
        function renderStoreRegistrations() {
            ...
            // แสดงเฉพาะร้านค้าที่มีสถานะ pending (read-only view)
            ...
            // Render history - แสดงประวัติทั้งหมด (read-only)
        ...
        function renderActiveStores() {
            ...
            // กรองร้านค้าตามสถานะ (active เท่านั้น)
            const activeStoresOnly = activeStores.filter(store => store.status === 'active');

            tbody.innerHTML = activeStoresOnly.map(store => {
                ...
                const userPassDisplay = `<div class="user-pass-container">
                    ...
                    <div class="user-pass-item">
                        ...
                        <span class="user-pass-value">${store.password}</span>
                    </div>
                </div>`;
                ...
                return `
                    <tr class="${isExpired ? 'expired-row' : ''} ${store.isPaused ? 'paused-row' : ''}">
                        ...
                        <td><span class="serial-key-cell">${store.serialKey}</span></td>
                        <td>
                            <div id="active-countdown-${store.id}" class="inline-countdown ${isExpired ? 'danger' : ''}"></div>
                        </td>
                        <td><span class="package-badge ${store.packageType}">${store.packageType === 'premium' ? '⭐ Premium' : '📦 Standard'}</span></td>
                        <td>
                            <span class="status-badge ${isExpired ? 'expired' : (store.isPaused ? 'paused' : 'active')}">
                                ${isExpired ? '❌ หมดอายุ' : (store.isPaused ? '⏸️ ระงับ' : '✓ เปิดใช้งาน')}
                            </span>
                        </td>
                        <td>
                            <div class="action-btn-group">
                                <button class="action-btn view" onclick="ManagerStore.viewStoreDashboard(${store.id})">👁️ Dashboard</button>
                                <button class="action-btn save" onclick="ManagerStore.confirmEditStore(${store.id})">💾 บันทึก</button>
                                <button class="action-btn delete" onclick="ManagerStore.confirmDeleteStore(${store.id})">🗑️ ลบ</button>
                            </div>
                        </td>
                    </tr>
                `;
            }).join('') || '<tr><td colspan="7" style="text-align:center; padding: 30px;">ไม่มีร้านค้าที่เปิดใช้งาน</td></tr>';

            // Start real-time countdown for each active store
            activeStoresOnly.forEach(store => {
                startRealTimeCountdown(`active-countdown-${store.id}`, store.expiryDate, store.id);
            });
        }

        // Real-time countdown with auto-lock functionality
        function startRealTimeCountdown(elementId, expiryDate, storeId) {
            const element = document.getElementById(elementId);
            if (!element) return;
            ...
            function updateCountdown() {
                ...
                if (diff <= 0) {
                    // เวลาหมดแล้ว - อัตโนมัติเปลี่ยนสถานะเป็น locked
                    autoLockStore(storeId);
                    element.innerHTML = '<span style="color: #e74c3c;">หมดอายุแล้ว</span>';
                    return;
                }
                ...
                let countdownText = '';
                if (years > 0) countdownText += `${years}ปี `;
                if (months > 0) countdownText += `${months}เดือน `;
                if (days > 0) countdownText += `${days}วัน `;
                countdownText += `${hours}ชม. ${minutes}น. ${seconds}วิ.`;

                element.innerHTML = `<span style="color: ${diff < 86400000 ? '#e74c3c' : '#27ae60'};">${countdownText}</span>`;
            }
        ...
        // Auto-lock store when time expires
        function autoLockStore(storeId) {
            const store = activeStores.find(s => s.id === storeId);
            if (!store) return;
            ...
            fetch(API_UPDATE_STORE_ENDPOINT, {
              ...
              .then(data => console.log('Auto-locked store:', data))
              .catch(err => console.error('Auto-lock Error:', err));

            // อัพเดท UI
            renderActiveStores();
            saveManagerData();

            Notify.warning('ระบบล็อคอัตโนมัติ', `ร้าน "${store.shopName}" ถูกล็อคเนื่องจากหมดอายุ`);
        }
        ...
        function showOpenStoreModal(storeId) {
            ...
            // Auto-fill ข้อมูลทั้งหมดจากข้อมูลการสมัคร
            const storeNameInput = document.getElementById('store-display-name');
            const usernameInput = document.getElementById('store-username');
            const passwordInput = document.getElementById('store-password');
            const serialKeyInput = document.getElementById('store-serial-key');
            const packageInput = document.getElementById('store-package');
            const timeRemainingInput = document.getElementById('store-time-remaining');

            // ดึงข้อมูลอัตโนมัติ
            if (storeNameInput) storeNameInput.value = store.shopName || '';
            if (usernameInput) usernameInput.value = store.username || store.ownerName || '';
            if (passwordInput) passwordInput.value = store.password || '';
            if (serialKeyInput) serialKeyInput.value = store.serialKey || '';
            if (packageInput) packageInput.value = store.packageType === 'premium' ? '⭐ Premium' : '📦 Standard';

            // คำนวณเวลาคงเหลือ
            if (timeRemainingInput && store.expiryDate) {
                const now = new Date().getTime();
                const expiry = new Date(store.expiryDate).getTime();
                const diff = expiry - now;
                ...
                if (diff > 0) {
                    ...
                    const hours = Math.floor((diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
                    const minutes = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
                    timeRemainingInput.value = `${days} วัน ${hours} ชั่วโมง ${minutes} นาที`;
                } else {
                    timeRemainingInput.value = 'หมดอายุแล้ว';
                }
            }
        ...
        function confirmOpenStore() {
            const modal = document.getElementById('open-store-modal');
            const storeId = parseInt(modal.dataset.storeId);
            const username = document.getElementById('new-store-username')?.value;
            const password = document.getElementById('new-store-password')?.value;
            ...
            // Clear inputs
            document.getElementById('new-store-username').value = '';
        ...
        function showRegistrationDetails(storeId) {
            ...
            if (content) {
                // คำนวณอายุร้าน (ปี/เดือน)
                const registeredDate = new Date(store.registeredAt);
                const now = new Date();
                const diffMonths = (now.getFullYear() - registeredDate.getFullYear()) * 12 + (now.getMonth() - registeredDate.getMonth());
                const years = Math.floor(diffMonths / 12);
                const months = diffMonths % 12;
                const ageText = years > 0 ? `${years} ปี ${months} เดือน` : `${months} เดือน`;

                // หาช่องทางติดต่อ (แสดง 1 ช่องทางที่มี)
                const contactMethods = [];
                if (store.email) contactMethods.push(`อีเมล: ${store.email}`);
                if (store.phone) contactMethods.push(`โทรศัพท์: ${store.phone}`);
                if (store.lineId) contactMethods.push(`LINE ID: ${store.lineId}`);
                if (store.facebook) contactMethods.push(`Facebook: ${store.facebook}`);
                const contactText = contactMethods.length > 0 ? contactMethods[0] : 'ไม่ระบุ';

                content.innerHTML = `
                    <div class="registration-detail-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; padding: 15px;">
                        <div class="detail-section">
                            <h4 style="margin-bottom: 15px; color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">🏪 ข้อมูลร้านค้า</h4>
                            <p><strong>ชื่อร้าน:</strong> ${store.shopName}</p>
                            <p><strong>อายุร้าน:</strong> ${ageText}</p>
                            ${store.shopUrl ? `<p><strong>ลิงก์ร้านหลัก:</strong> <a href="${store.shopUrl}" target="_blank" style="color: var(--primary-color);">${store.shopUrl}</a></p>` : ''}
                        </div>
                        
                        <div class="detail-section">
                            <h4 style="margin-bottom: 15px; color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">👤 บัญชีผู้ใช้</h4>
                            <p><strong>Username:</strong> ${store.username || store.ownerName || '-'}</p>
                            <p><strong>Password:</strong> ${store.password || 'ไม่ระบุ'}</p>
                        </div>
                        
                        <div class="detail-section">
                            <h4 style="margin-bottom: 15px; color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">📞 ช่องทางติดต่อ</h4>
                            <p>${contactText}</p>
                        </div>
                        
                        <div class="detail-section">
                            <h4 style="margin-bottom: 15px; color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">📦 แพ็คเกจ</h4>
                            <p><strong>แพ็คเกจที่เลือก:</strong> <span class="package-badge ${store.packageType || 'standard'}">${store.packageType === 'premium' ? '⭐ Premium' : '📦 Standard'}</span></p>
                        ...
                        <div class="detail-section" style="grid-column: 1 / -1;">
                            <h4 style="margin-bottom: 15px; color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">🔑 Serial Key</h4>
        ...
        function confirmEditStore(storeId) {
            const store = activeStores.find(s => s.id === storeId);
            if (!store) return;

            // แสดง modal แก้ไขข้อมูลร้านค้า
            const modal = document.getElementById('edit-store-modal') || createEditStoreModal();
            modal.dataset.storeId = storeId;

            // กรอกข้อมูลปัจจุบัน
            document.getElementById('edit-store-name').value = store.shopName;
            document.getElementById('edit-store-username').value = store.username;
            document.getElementById('edit-store-password').value = store.password;
            document.getElementById('edit-store-package').value = store.packageType;
        ...
        function saveEditedStore() {
            ...
            // อัปเดตข้อมูล
            store.shopName = document.getElementById('edit-store-name').value;
            store.username = document.getElementById('edit-store-username').value;
            store.password = document.getElementById('edit-store-password').value;
            store.packageType = document.getElementById('edit-store-package').value;
            ...
            fetch(API_UPDATE_STORE_ENDPOINT, {
              ...
              .catch(err => console.error('Update store error:', err));

            renderActiveStores();
            saveManagerData();

            Notify.success('บันทึกสำเร็จ', 'ข้อมูลร้านค้าถูกบันทึกเรียบร้อยแล้ว');
            ...
            modal.innerHTML = `
                <div class="modal-content">
                    ...
                    <form id="edit-store-form">
                        <div class="form-group">
                            <label for="edit-store-name">ชื่อร้านค้า</label>
                            <input type="text" id="edit-store-name" required>
                        </div>
                        <div class="form-group">
                            <label for="edit-store-username">Username</label>
                            <input type="text" id="edit-store-username" required>
                        </div>
                        <div class="form-group">
                            <label for="edit-store-password">Password</label>
                            <input type="password" id="edit-store-password" required>
                        </div>
                        <div class="form-group">
                            <label for="edit-store-package">แพ็คเกจ</label>
                            <select id="edit-store-package">
                                <option value="standard">📦 Standard</option>
                                <option value="premium">⭐ Premium</option>
                            </select>
                        </div>
                        <div class="modal-actions">
                            <button type="button" onclick="ManagerStore.saveEditedStore()" class="btn btn-success">💾 บันทึก</button>
                            <button type="button" onclick="this.closest('.modal').style.display='none'" class="btn btn-secondary">ยกเลิก</button>
                        </div>
                    </form>
                </div>
            `;
        ...
        function showSubMenu(menuName) {
            ...
            switch(menuName) {
                ...
                case 'payment-stores':
                    renderPaymentChannels();
                    renderPaymentHistory();
                    syncPaymentData(); // Two-way sync
                    renderPaymentStoresAdmin(); // Admin view with two-way sync
                    break;
        ...
        async function init() {
            ...
            // Set up payment manager modal
            setupPaymentManagerModal();
        ...
        function setupPaymentManagerModal() {
            const closeBtn = document.getElementById('close-store-payment-manager-btn');
            if (closeBtn) {
        ...
        return {
            ...
            // Payment Manager (Store View) & Payment Stores (Admin View)
            renderStorePaymentManager,
            renderPaymentStoresAdmin,
            syncPaymentData,
            updateStorePaymentAmount,
            attachPaymentProof,
