...
cartDetails.addEventListener('click', (e) => {
    ...
    const ManagerStore = (() => {
        ...
        // Auto-lock intervals storage
        let autoLockInterval = null;

        // ===== Auto-Lock Logic =====
        function startAutoLockCheck() {
            // Check every 30 seconds
            autoLockInterval = setInterval(checkExpiredStores, 30000);
            console.log('Auto-lock check started');
        }
        ...
        function stopAutoLockCheck() {
            if (autoLockInterval) {
                ...
                console.log('Auto-lock check stopped');
            }
        }

        function checkExpiredStores() {
            const now = new Date().getTime();
            let hasExpiredStores = false;

            activeStores.forEach(store => {
                const expiry = new Date(store.expiryDate).getTime();
                if (now >= expiry && store.status !== 'locked') {
                    // Store expired - lock it
                    store.status = 'locked';
                    store.lockedAt = new Date().toISOString();
                    store.isOnline = false;
                    hasExpiredStores = true;
                    
                    console.log(`Store "${store.shopName}" has been auto-locked due to expiry`);
                    
                    // If this store is currently logged in, show lock overlay
                    if (currentStoreSession && currentStoreSession.id === store.id) {
                        showSystemLockOverlay('ร้านค้าหมดอายุ', 'กรุณาติดต่อ IT เพื่อปลดล็อคระบบ');
                    }
                }
            });
            ...
            if (hasExpiredStores) {
                ...
                Notify.warning('ระบบล็อคอัตโนมัติ', 'มีร้านค้าที่หมดอายุและถูกล็อคอัตโนมัติ');
            }
        }
        ...
        function unlockStore(storeId) {
            ...
            Notify.success('ปลดล็อคสำเร็จ', `ร้าน "${store.shopName}" สามารถใช้งานได้แล้ว`);
            return true;
        }
        ...
        function addStoreRegistration(registration) {
            ...
            registration.status = 'request_pending'; // สถานะใหม่: รอการอนุมัติ
            registration.registeredAt = new Date().toISOString();
            storeRegistrations.push(registration);
            renderStoreRegistrations();
            saveManagerData();
            Notify.success('เพิ่มร้านค้าสำเร็จ', `ร้าน "${registration.shopName}" ถูกเพิ่มเข้าระบบแล้ว (สถานะ: request_pending)`);
        }
        ...
        function approveRegistration(id, packageType) {
            ...
            store.status = 'approved_waiting_setup'; // สถานะใหม่: รอการตั้งค่า (Serial Key + User/Pass)
            store.packageType = packageType;
            store.approvedAt = new Date().toISOString();

            // Move to pending stores (waiting for serial key and setup)
            pendingStores.push({...store, serialKey: null, username: null, password: null, permissions: []});
            ...
            fetch(API_UPDATE_STORE_ENDPOINT, {
                ...
                body: JSON.stringify({
                    ...
                    status: 'approved_waiting_setup' // สถานะใหม่
                })
            }).then(res => res.json())
            ...
            Notify.success('อนุมัติสำเร็จ', `ร้าน "${store.shopName}" ได้รับการอนุมัติแล้ว (สถานะ: approved_waiting_setup)`);
        }
        ...
        function rejectRegistration(id) {
            ...
            const store = storeRegistrations[index];
            store.status = 'rejected'; // สถานะใหม่: ถูกปฏิเสธ
            ...
            Notify.warning('ปฏิเสธการสมัคร', `ร้าน "${store.shopName}" ถูกปฏิเสธแล้ว (สถานะ: rejected)`);
        }
        ...
        function openNewStore(storeId, username, password, selectedSerialKeyId, permissions) {
            const storeIndex = pendingStores.findIndex(s => s.id === storeId);
            if (storeIndex === -1) return;

            const store = pendingStores[storeIndex];
            if (!selectedSerialKeyId) {
                Notify.error('ไม่สามารถเปิดร้านได้', 'กรุณาเลือก Serial Key จาก Stock');
                return;
            }

            // หา Serial Key ที่เลือก
            const serialKey = serialKeys.find(k => k.id === parseInt(selectedSerialKeyId));
            if (!serialKey) {
                Notify.error('ไม่สามารถเปิดร้านได้', 'Serial Key ที่เลือกไม่ถูกต้อง');
                return;
            }

            store.username = username;
            store.password = password;
            store.serialKey = serialKey.key; // ใช้ Serial Key ที่เลือก
            store.expiryDate = serialKey.expiryDate; // วันหมดอายุจาก Serial Key
            store.permissions = permissions || []; // สิทธิ์การเข้าถึง
            store.status = 'active';
            store.openedAt = new Date().toISOString();
            store.isOnline = false;
            store.isPaused = false;

            // อัพเดท Serial Key status
            serialKey.status = 'used';
            serialKey.assignedTo = store.id;
            ...
            fetch(API_UPDATE_STORE_ENDPOINT, {
                ...
                body: JSON.stringify({
                    ...
                    serialKeyId: selectedSerialKeyId,
        ...
        function storeLogin(username, password) {
            ...
            if (store.status === 'locked') {
                showSystemLockOverlay('ร้านค้าถูกล็อค', 'คุณค้างค่าใช้จ่ายระบบ ติดต่อ IT เพื่อปลดล็อคระบบ');
                return false;
            }
            ...
            if (now >= expiry) {
                ...
                showSystemLockOverlay('ร้านค้าหมดอายุ', 'คุณค้างค่าใช้จ่ายระบบ ติดต่อ IT เพื่อปลดล็อคระบบ');
                return false;
            }
        ...
        // ===== System Lock Overlay =====
        function showSystemLockOverlay(title = 'ระบบถูกระงับการใช้งาน', message = 'กรุณาติดต่อผู้ดูแลระบบ') {
            const overlay = document.getElementById('system-lock-overlay');
            if (overlay) {
                overlay.style.display = 'flex';
                
                // Update lock message
                const messageEl = overlay.querySelector('.system-lock-message h2');
                const descEl = overlay.querySelector('.system-lock-message p');
                
                if (messageEl) messageEl.textContent = title;
                if (descEl) descEl.textContent = message;
                
                // Store logout if currently logged in
                if (currentStoreSession) {
                    storeLogout();
                }
            }
        ...
        function renderStoreRegistrations() {
            ...
            // แสดงเฉพาะร้านค้าที่สถานะ 'request_pending' (รอการอนุมัติ)
            const pendingRequests = storeRegistrations.filter(s => s.status === 'request_pending');
            
            tbody.innerHTML = pendingRequests.map(store => `
                <tr>
                    ...
                    <td>
                        <span class="status-badge pending">รอดำเนินการ (request_pending)</span>
                    </td>
                    <td>
            ...
            `).join('') || '<tr><td colspan="5" style="text-align:center; padding: 30px;">ไม่มีคำร้องร้านใหม่ (request_pending)</td></tr>';

            // Render history (แสดงประวัติทั้งหมดยกเว้น request_pending)
            const historyContainer = document.getElementById('store-registrations-history-table');
            if (historyContainer) {
                const historyTbody = historyContainer.querySelector('tbody');
                if (historyTbody) {
                    const historyData = storeRegistrations.filter(s => s.status !== 'request_pending');
                    
                    historyTbody.innerHTML = historyData.map(store => `
                        <tr>
                            <td>${store.shopName}</td>
                            <td>${new Date(store.registeredAt).toLocaleDateString('th-TH')}</td>
                            <td>
                            ...
                            <td>
                                <span class="status-badge ${store.status}">
                                    ${getStatusDisplayName(store.status)}
                                </span>
                            </td>
                            <td>
                                <button class="action-btn view" onclick="ManagerStore.showRegistrationDetails(${store.id})">
                    ...
                    `).join('') || '<tr><td colspan="5" style="text-align:center; padding: 30px;">ไม่มีประวัติ</td></tr>';
                }
            }
        }
        ...
        function renderPendingStores() {
            // แสดงเฉพาะร้านค้าที่สถานะ 'approved_waiting_setup' (รอการตั้งค่า)
            const readyToSetupStores = pendingStores.filter(store => store.status === 'approved_waiting_setup');
            ...
            tbody.innerHTML = readyToSetupStores.map(store => `
                <tr>
                    <td>${store.shopName}</td>
                    <td><span class="package-badge ${store.packageType}">${store.packageType === 'premium' ? '⭐ Premium' : '📦 Standard'}</span></td>
                    <td>
                        ${store.serialKey ? 
                            `<span class="serial-key-cell">${store.serialKey}</span>` : 
                            '<span style="color: var(--text-secondary);">ยังไม่ได้กำหนด</span>'
                        }
                    </td>
                    <td>
                        ${store.expiryDate ? 
                            `<div id="open-store-countdown-${store.id}" class="countdown-display" style="transform: scale(0.7); transform-origin: left;"></div>` :
                            '<span style="color: var(--text-secondary);">รอกำหนด Serial Key</span>'
                        }
                    </td>
                    <td>
                        <div class="action-btn-group">
            ...
            `).join('') || '<tr><td colspan="5" style="text-align:center; padding: 30px;">ไม่มีร้านค้าที่รอการตั้งค่า (approved_waiting_setup)</td></tr>';

            // Start countdown for stores with expiry date
            readyToSetupStores.forEach(store => {
                if (store.expiryDate) {
        ...
        function renderActiveStores() {
            ...
            tbody.innerHTML = activeStores.map(store => {
                ...
                // กำหนดสถานะและปุ่มที่แสดง
                let statusDisplay = '';
                let actionsHtml = '';
                
                if (isLocked) {
                    statusDisplay = `<span class="status-badge locked">🔒 ถูกล็อค</span>`;
                    actionsHtml = `
                        <button class="action-btn edit" onclick="ManagerStore.confirmUnlockStore(${store.id})">🔓 ปลดล็อค</button>
                        <button class="action-btn delete" onclick="ManagerStore.confirmDeleteStore(${store.id})">🗑️ ลบ</button>
                    `;
                } else if (isExpired) {
                    statusDisplay = `<span class="status-badge expired">❌ หมดอายุ</span>`;
                    actionsHtml = `
                        <button class="action-btn view" onclick="ManagerStore.viewStoreDashboard(${store.id})">👁️ Dashboard</button>
                        <button class="action-btn delete" onclick="ManagerStore.confirmDeleteStore(${store.id})">🗑️ ลบ</button>
                    `;
                } else if (store.isPaused) {
                    statusDisplay = `<span class="status-badge paused">⏸️ ระงับ</span>`;
                    actionsHtml = `
                        <button class="action-btn view" onclick="ManagerStore.viewStoreDashboard(${store.id})">👁️ Dashboard</button>
                    ...
                    statusDisplay = `<span class="status-badge active">✓ เปิดใช้งาน</span>`;
                    actionsHtml = `
                        <button class="action-btn view" onclick="ManagerStore.viewStoreDashboard(${store.id})">👁️ Dashboard</button>
        ...
        function getStatusDisplayName(status) {
            switch(status) {
                case 'request_pending':
                    return 'รอดำเนินการ';
                case 'approved_waiting_setup':
                    return 'อนุมัติแล้ว (รอตั้งค่า)';
                case 'rejected':
                    return 'ไม่อนุมัติ';
                case 'active':
                    return 'เปิดใช้งาน';
        ...
        function showOpenStoreModal(storeId) {
            ...
            // Populate Serial Key selector with unused keys
            const serialKeySelector = document.getElementById('store-serial-key-selector');
            if (serialKeySelector) {
                // ล้างตัวเลือกเดิม
                serialKeySelector.innerHTML = '<option value="">-- เลือก Serial Key ที่มีอยู่ --</option>';
                
                // เพิ่มตัวเลือก Serial Key ที่ยังไม่ได้ใช้
                serialKeys.filter(k => k.status === 'unused').forEach(key => {
                    const option = document.createElement('option');
                    option.value = key.id;
                    option.textContent = `${key.key} (${getDurationLabel(key.duration)}) - หมดอายุ: ${new Date(key.expiryDate).toLocaleDateString('th-TH')}`;
                    serialKeySelector.appendChild(option);
                });
            }
            ...
            // Display selected Serial Key info when user selects
            if (serialKeySelector) {
                serialKeySelector.addEventListener('change', function() {
                    const selectedKeyId = this.value;
                    const selectedKey = serialKeys.find(k => k.id === parseInt(selectedKeyId));
                    const displayEl = document.getElementById('store-serial-key-display');
                    
                    if (selectedKey && displayEl) {
                        displayEl.value = selectedKey.key;
                        // Update countdown display
                        const countdownContainer = document.getElementById('store-countdown-display');
                        if (countdownContainer) {
                            startCountdown('store-countdown-display', selectedKey.expiryDate);
                        }
                    } else if (displayEl) {
                        displayEl.value = '';
                    }
        ...
        function confirmOpenStore() {
            ...
            const serialKeySelector = document.getElementById('store-serial-key-selector');
            const selectedSerialKeyId = serialKeySelector?.value;
            ...
            if (!selectedSerialKeyId) {
                Notify.error('กรุณาเลือก Serial Key', 'กรุณาเลือก Serial Key จาก Stock');
                return;
            }

            // รวบรวม permissions ที่เลือก
            const selectedPermissions = [];
            document.querySelectorAll('input[name="permissions"]:checked').forEach(checkbox => {
                selectedPermissions.push(checkbox.value);
            });

            openNewStore(storeId, username, password, selectedSerialKeyId, selectedPermissions);
            hideOpenStoreModal();
            ...
            if (serialKeySelector) {
                serialKeySelector.value = '';
            }
            document.getElementById('store-serial-key-display').value = '';
        }
        ...
        function confirmUnlockStore(storeId) {
            const store = activeStores.find(s => s.id === storeId);
            Notify.confirm({
                title: 'ยืนยันการปลดล็อค',
                message: `คุณต้องการปลดล็อคร้าน "${store?.shopName}" หรือไม่?`,
                confirmText: 'ปลดล็อค',
                cancelText: 'ยกเลิก',
                type: 'info'
            }).then(confirmed => {
                if (confirmed) {
                    if (unlockStore(storeId)) {
                        renderActiveStores();
                    }
                }
            });
        ...
        function showSubMenu(menuName) {
            // Update active submenu in global state
            if (window.activeAdminSubMenus) {
                window.activeAdminSubMenus['manager-store'] = menuName;
            }
            ...
            switch(menuName) {
                case 'store-registrations':
                    ...
                    break;
                case 'open-new-store':
                    // แสดงเฉพาะร้านค้าที่สถานะ approved_waiting_setup
                    renderPendingStores();
                    break;
                ...
                case 'payment-stores':
                    ...
                    break;
                case 'store-payment':
                    // แสดง Payment Manager สำหรับร้านค้า
                    if (currentStoreSession) {
                        renderStorePaymentDashboard(currentStoreSession);
                    }
                    break;
            }
        }
        ...
        async function init() {
            ...
            // Set up open store modal
            setupOpenStoreModal();

            // Setup Manager Store tabs
            setupManagerStoreTabs();
        ...
        function setupManagerStoreTabs() {
            const tabsContainer = document.getElementById('manager-store-tabs');
            if (!tabsContainer) return;

            // กำหนดโครงสร้าง tabs ตามที่ผู้ใช้ต้องการ
            tabsContainer.innerHTML = `
                <div class="tab-group">
                    <button class="tab ${!activeAdminSubMenus['manager-store'] || activeAdminSubMenus['manager-store'] === 'store-registrations' ? 'active' : ''}" 
                            data-submenu="store-registrations" onclick="ManagerStore.showSubMenu('store-registrations')">
                        📋 คำร้องร้านใหม่
                    </button>
                    <button class="tab ${activeAdminSubMenus['manager-store'] === 'open-new-store' ? 'active' : ''}" 
                            data-submenu="open-new-store" onclick="ManagerStore.showSubMenu('open-new-store')">
                        🏪 เปิดร้านค้าใหม่
                    </button>
                    <button class="tab ${activeAdminSubMenus['manager-store'] === 'track-operations' ? 'active' : ''}" 
                            data-submenu="track-operations" onclick="ManagerStore.showSubMenu('track-operations')">
                        👁️ ติดตามการทำงาน
                    </button>
                    <button class="tab ${activeAdminSubMenus['manager-store'] === 'payment-stores' ? 'active' : ''}" 
                            data-submenu="payment-stores" onclick="ManagerStore.showSubMenu('payment-stores')">
                        💰 Payment Stores
                    </button>
                </div>
            `;
        ...
        function systemEdit() {
            Notify.confirm({
                title: 'ระบบทำความสะอาด',
                message: 'คุณต้องการทำความสะอาดระบบและจัดเรียงเมนูใหม่หรือไม่?',
        ...
        function performSystemCleanup() {
            try {
                // Clean up menu order based on current config
                if (window.appData && window.appData.menuOrder) {
                    // Remove any duplicate entries
                    const uniqueMenuOrder = [...new Set(window.appData.menuOrder)];
                    window.appData.menuOrder = uniqueMenuOrder;
                    
                    // Ensure required menus exist
                    const requiredMenus = ['dashboard', 'stock', 'admin'];
                    requiredMenus.forEach(menu => {
                        if (!uniqueMenuOrder.includes(menu)) {
                            uniqueMenuOrder.push(menu);
                        }
                    });
                ...
                activeStores = activeStores.filter(store => {
                    // Remove stores with invalid data
                    if (!store.id || !store.shopName || !store.username) {
                        console.log(`Removed invalid store: ${store.shopName || 'Unknown'}`);
                        return false;
                    }
                    return true;
                });

                // Clean up expired stores that are older than 30 days
                const thirtyDaysAgo = new Date(Date.now() - (30 * 24 * 60 * 60 * 1000));
                const originalLength = activeStores.length;
                
                activeStores = activeStores.filter(store => {
                    if (store.status === 'locked' && store.lockedAt) {
                        const lockedDate = new Date(store.lockedAt);
                        if (lockedDate < thirtyDaysAgo) {
                            console.log(`Removed old locked store: ${store.shopName}`);
                            return false;
                        }
                    }
                    return true;
                ...
                // Clean up payment history older than 90 days
                ...
                const removedStores = originalLength - activeStores.length;
                const message = removedStores > 0 
                    ? `ทำความสะอาดสำเร็จ! ลบร้านค้าที่ไม่ถูกต้อง ${removedStores} ร้าน`
                    : 'ทำความสะอาดสำเร็จ! ไม่พบข้อมูลที่ต้องลบ';

                Notify.success('ระบบทำความสะอาดเสร็จสิ้น', message);

                // Add log entry
                if (typeof addLog === 'function') {
                    addLog('System Cleanup', `Cleaned up system data. Removed ${removedStores} invalid stores.`);
                }

            } catch (error) {
                console.error('System cleanup error:', error);
                Notify.error('เกิดข้อผิดพลาด', 'ไม่สามารถทำความสะอาดระบบได้');
            }
        }

        // ===== Data Isolation Functions =====
        function getStoreData(storeId) {
            // Return only data specific to this store
            const store = activeStores.find(s => s.id === storeId);
            if (!store) return null;

            return {
                storeInfo: store,
                // Filter data by storeId where applicable
                products: appData.products?.filter(p => p.storeId === storeId) || [],
                categories: appData.categories?.filter(c => c.storeId === storeId) || [],
                orders: appData.orders?.filter(o => o.storeId === storeId) || [],
                customers: appData.customers?.filter(c => c.storeId === storeId) || []
            };
        }
    ...
    async function initializeAllSystems() {
        try {
            ...
            // Setup Manager Store event listeners
            setupManagerStoreEventListeners();
    ...
    // Setup Manager Store specific event listeners
    function setupManagerStoreEventListeners() {
        // System Edit button
        const editBtn = document.getElementById('system-edit-btn');
        if (editBtn) {
            editBtn.addEventListener('click', () => {
                if (window.ManagerStore && ManagerStore.systemEdit) {
                    ManagerStore.systemEdit();
                }
            });
        }

        // Store login form
        const storeLoginForm = document.getElementById('store-login-form');
        if (storeLoginForm) {
            storeLoginForm.addEventListener('submit', (e) => {
                e.preventDefault();
                const username = document.getElementById('store-login-username').value;
                const password = document.getElementById('store-login-password').value;

                if (window.ManagerStore && ManagerStore.storeLogin) {
                    const success = ManagerStore.storeLogin(username, password);
                    if (success) {
                        // Hide login overlay and show store payment dashboard
                        hideStoreLoginOverlay();
                        if (window.ManagerStore && ManagerStore.showSubMenu) {
                            ManagerStore.showSubMenu('store-payment');
                        }
                    }
                }
            });
        ...
        // Logout button in lock overlay
        const logoutBtn = document.getElementById('logout-btn');
        if (logoutBtn) {
            logoutBtn.addEventListener('click', () => {
                hideSystemLockOverlay();
                hideStoreLoginOverlay();
                if (window.ManagerStore && ManagerStore.storeLogout) {
                    ManagerStore.storeLogout();
                }
            });
        }
    }

    function hideStoreLoginOverlay() {
        const overlay = document.getElementById('store-login-overlay');
        if (overlay) {
            overlay.style.display = 'none';
        }
    }

    function showStoreLoginOverlay() {
        const overlay = document.getElementById('store-login-overlay');
        if (overlay) {
            overlay.style.display = 'flex';
            // Clear form
            document.getElementById('store-login-username').value = '';
